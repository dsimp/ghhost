...
export default function NFLDashboard() {
  ...
  const [targetStat, setTargetStat] = useState('PASS_YDS'); 
  ...
  useEffect(() => {
    Promise.all([
      fetch('/api/nfl/players').then(res => res.json()),
      fetch('/api/nfl/teams').then(res => res.json())
  ...
  const runRiskEngine = async (playerId, opponentName, opponentAbbr, isHomeGame) => {
    ...
    try {
      const statsRes = await fetch(`/api/nfl/playerStats?playerId=${playerId}`);
  ...
  const runSpatialEngine = async (playerId, opponentId, playerName) => {
     ...
     try {
       const [playerRes, defRes] = await Promise.all([
          fetch(`/api/nfl/shotChart?playerId=${playerId}`),
          fetch(`/api/nfl/shotChart?teamId=${opponentId}&defenseMode=true`)
       ]);
    ...
    try {
      if (mode === 'PLAYER') {
        const [statsRes, shotRes] = await Promise.all([
          fetch(`/api/nfl/playerStats?playerId=${entity.id}`),
          fetch(`/api/nfl/shotChart?playerId=${entity.id}`)
        ]);
        const stats = await statsRes.json();
        ...
        const shotRes = await fetch(`/api/nfl/shotChart?teamId=${entity.id}&defenseMode=true`);
        const shots = await shotRes.json();
  ...
  const loadPredictor = async () => {
     ...
     setError('');
     try {
       const res = await fetch('/api/nfl/predictToday');
    ...
    <main className="sport-layout-container">
      ...
      <aside className="sport-sidebar">
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px', padding: '0 8px' }}>
          ...
          <div>
            <h1 style={{ fontSize: '1.4rem', margin: 0, textTransform: 'uppercase', letterSpacing: '0.05em', color: 'var(--text-main)' }}>NFL</h1>
                      ...
                      <div style={{
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '600px', overflowY: 'auto' }}>
                          {(expandedOvers ? oversBoard : oversBoard.slice(0, 4)).map((gb, i) => (
                            <ExplainerCard key={`over-${i}`} prediction={gb} sport="NFL" overlayMode={true} triggerType="wrap">
                              <div style={{
                      ...
                      <div style={{
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '600px', overflowY: 'auto' }}>
                          {(expandedUnders ? undersBoard : undersBoard.slice(0, 4)).map((gb, i) => (
                            <ExplainerCard key={`under-${i}`} prediction={gb} sport="NFL" overlayMode={true} triggerType="wrap">
                              <div style={{
                                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                                background: 'rgba(0,0,0,0.3)', borderRadius: '10px', padding: '10px 14px',
                                 ...
                                 <>
                                   <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '20px' }}>
                                     <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                       ...
                                       >
                                         {['PASS YDS','RUSH YDS','REC YDS','PASS TDS','RUSH TDS','REC TDS','COMP','REC','INT','SACKS','TACKLES'].map(s => <option key={s} value={s}>{s}</option>)}
                                       </select>
                                     </div>
                                   </div>
